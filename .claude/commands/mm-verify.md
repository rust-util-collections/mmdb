...
- Output from `/review` that needs validation
...
- [ ] **FP-2**: Is the relevant lock held for the entire check-then-act? If so, no TOCTOU race.
- [ ] **FP-3**: Is ArcSwap staleness intentional for this code path?
- [ ] **FP-4**: Is the unwrap() on a known-valid state?
- [ ] **FP-5**: Would clippy catch this? If so, CI already enforces it.
...
- [ ] **FP-10**: Has the full locking protocol been analyzed?
- [ ] **FP-11**: Is tombstone retention correct by design?
- [ ] **FP-12**: Has the half-open interval been verified at the point of use?
