...
impl IterSource {
    ...
    /// tombstone may cover keys from its own level or deeper, but a tombstone
    /// strictly deeper than a key's source level must never cover it. Sources
    /// left untagged (`usize::MAX`) disable the level filter for
